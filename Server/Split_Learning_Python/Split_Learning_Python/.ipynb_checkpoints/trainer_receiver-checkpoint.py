    ...
    def run(self):
        while True:
            ...
            if status == 1:
                ...
                reply = pickle.dumps(server_reply)
...
def train_in_server(outputs_mobile, labels, layer_no):
    ...
    net_server.layer8.register_backward_hook(printnorm)
...
# Server Side
if (path.exists("server.pkl") == False):
    net_server = AlexNet_Server()
else:
    net_server = torch.load("server.pkl")
